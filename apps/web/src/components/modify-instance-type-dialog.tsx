...
const T4G_INSTANCE_TYPES = [
  ...
  {
    value: 't4g.xlarge',
    label: 't4g.xlarge (4 vCPU, 16 GB)',
    vcpu: 4,
    memory: 16,
  },
  {
    value: 't4g.2xlarge',
    label: 't4g.2xlarge (8 vCPU, 32 GB)',
    vcpu: 8,
    memory: 32,
  },
];
  ...
  const { data: instanceTypes, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const typeGroups: Record<
        string,
        Array<{ value: string; label: string; vcpu: number; memory: number }>
      > = {};
      ...
      types.forEach((type) => {
        ...
        const memory = type.MemoryInfo?.SizeInMiB
          ? type.MemoryInfo.SizeInMiB / 1024
          : 0;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Change instance type for {instance.instanceName} (
            {instance.instanceId})
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <Alert>
            ...
            <AlertDescription>
              <strong>Note:</strong> Instance must be in stopped state to modify
              type. Instance needs to be restarted after modification.
            </AlertDescription>
          </Alert>
